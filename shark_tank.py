...
def sharks_info(season_df,keys,shark_name, occupation, education,guest_list):
    if st.session_state.selected_shark == "Guests": 
        ...
        st.markdown(f"<h2 style='text-align: center;font-size: 16px;color: #888;'>Select a Shark(Guest) name</h2>", unsafe_allow_html=True)
        col700,col701,col703,col704,col705=st.columns([4,4,3,4,4])
        st.markdown(
        """
        <style>
...
def sharks(season_df,guests_list):
    st.markdown("""<h1  style='text-align: center; color: #FFD700;'>Shark Analysis</h1>""", unsafe_allow_html=True)
